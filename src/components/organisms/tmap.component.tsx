...
import MapAddressComponent from "../molecules/map.address.component";
...
const TMapComponent = () => {
  ...
  function drawHubs() {
    hubs.map((dest: any, index: number) => {
      addMarker("llHub", dest.viaX, dest.viaY, index, false, dest);
    });
  }
  ...
  function drawDestinations() {
    destionations.map((dest: any, index: number) => {
      addMarker("llPass", dest.viaX, dest.viaY, index, false, dest);
    });
  }
  ...
  function handleGetRoute(dest: any) {
    //기존 맵에 있던 정보들 초기화
    resettingMap();
    getRoute({
  ...
  return (
    <div>
      ...
      {destination ? (
        <MapAddressComponent {...destination} onGetRoute={handleGetRoute} />
      ) : (
        <Image
